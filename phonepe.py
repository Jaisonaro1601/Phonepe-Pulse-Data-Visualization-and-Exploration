...
def Transaction_amount_count_Y_Q(df,quarter):
    tacy=Aggre_insurance[Aggre_insurance["Quarter"]==quarter]
    tacy.reset_index(drop=True,inplace=True) 
    tacyg=tacy.groupby('States')[["Transaction_count","Transaction_amount"]].sum()
    tacyg.reset_index(inplace=True)
    col1,col2=st.columns(2)
...
def Aggre_user_plot_1(df,year):
    ...
    fig_bar_1=px.bar(aguyg,x="Brands",y="Transaction_count",title=f"{year} BRANDS AND TRANSACTION COUNT",width=800,color_discrete_sequence=px.colors.sequential.Agsunset_r,hover_name="Brands")
...
def Aggre_user_plot_2(df,quarter):
    ...
    fig_bar_1=px.bar(aguyqg,x="Brands",y="Transaction_count",title=f"{quarter} QUARTER, BRANDS AND TRANSACTION COUNT",width=800,color_discrete_sequence=px.colors.sequential.Agsunset_r)
    st.plotly_chart(fig_bar_1)
    return aguyq
...
if select=="Home":
    ...
    with col2:
        st.image(Image.open("E:\capstone_projects\download.jpeg"),width=400)
    col3,col4= st.columns(2)
    
    with col3:
        st.image(Image.open(r"E:\capstone_projects\images.jpeg"),width=400)
    ...
    with col6:
        st.image(Image.open(r"E:\capstone_projects\images (1).jpeg"),width= 400)
    ...
    with tab1:
        ...
        if method == "Insurance Analysis":
            ...
            col1,col2=st.columns(2)
            with col1:  
                years=st.slider("SELECT YEAR",Aggre_insurance["Years"].min(),Aggre_insurance["Years"].max(),Aggre_insurance["Years"].min())
            ...
            col1,col2=st.columns(2)
            with col1:
                states=st.selectbox("SELECT STATES", Aggre_tran_tac_Y["States"].unique())
            ...
            col1,col2=st.columns(2)
            with col1:
                states=st.selectbox("SELECT STATES_TY", Aggre_tran_tac_Y_Q["States"].unique())
            ...
            col1,col2=st.columns(2)
            with col1:
                states=st.selectbox("SELECT STATES", Aggre_user_Y_Q["States"].unique())
    ...
    with tab2:
        ...
        if method_2 == "Map Insurance Analysis":
            ...
            col1,col2= st.columns(2)
            with col1:
                states= st.selectbox("Select The State_nu", map_user_Y_Q["States"].unique())
